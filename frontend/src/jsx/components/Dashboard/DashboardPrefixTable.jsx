 ...
 */
const DashboardPrefixTable = ({ rows = [] }) => {
  if (!rows.length) {
    return <div className="klk-dash-empty">No company (prefix) data available yet.</div>;
  }
  ...
  const capacityTotals = rows.reduce((acc, item) => {
    (item.capacities || []).forEach(({ capacity, count }) => {
      acc[capacity] = (acc[capacity] || 0) + count;
    });
    return acc;
  }, {});

  const footerCapacities = Object.entries(capacityTotals)
  ...
  return (
    <div className="klk-cap-table-wrap">
      <table className="klk-cap-table">
        <thead>
          <tr>
            <th className="klk-cap-table__th-cap">Company (Prefix)</th>
            <th className="klk-cap-table__th-capacities">Capacities</th>
            <th>Total</th>
            <th>Produced</th>
            <th>Dispatched</th>
            <th>Collected</th>
        ...
        <tbody>
          {rows.map((item) => {
            ...
            return (
              <tr key={item._id}>
                <td>
                  <span className="klk-cap-badge klk-cap-badge--prefix">
                    <i className="fa-solid fa-building" aria-hidden="true" />
                    {item._id}
                  </span>
                </td>
                <td className="klk-cap-table__capacities">
                  {(item.capacities || []).length > 0 ? (
                    <div className="klk-prefix-cap-list">
                      {item.capacities.map(({ capacity, count }) => (
                        <span key={capacity} className="klk-prefix-cap-chip">
                          <span className="klk-prefix-cap-chip__label">{capacity} W</span>
                          <span className="klk-prefix-cap-chip__count">{count.toLocaleString()}</span>
                        </span>
                      ))}
                    </div>
                  ) : (
                    <span className="klk-cap-pill klk-cap-pill--muted">—</span>
                  )}
                </td>
        ...
        <tfoot>
          <tr>
            ...
            <td className="klk-cap-table__capacities">
              {footerCapacities.length > 0 ? (
                <div className="klk-prefix-cap-list">
                  {footerCapacities.map(({ capacity, count }) => (
                    <span key={capacity} className="klk-prefix-cap-chip klk-prefix-cap-chip--total">
                      <span className="klk-prefix-cap-chip__label">{capacity} W</span>
                      <span className="klk-prefix-cap-chip__count">{count.toLocaleString()}</span>
                    </span>
                  ))}
...
export default DashboardPrefixTable;
